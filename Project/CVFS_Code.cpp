...
int writeFile(int fd,char*arr,int isize)
{
 ...
 strncpy((UFDTArr[fd].ptrfiletable->ptrinode->Buffer)+(UFDTArr[fd].ptrfiletable->writeoffsetoffset),arr,isize);
...
int LseekFile(int fd,int size,int from)
{
        ...
        if(from = = CURRENT)
    {
        ...
        if(((UFDTArr[fd].ptrfiletable->writeoffset)+size)>(UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize))
            ...
            (UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize)=(UFDTArrfd.ptrfiletable->writeoffset)+size;
    }
    else if(from = = START)
    {
...
int truncate_File(char *name)
{
    ...
    UFDTArr[fd].ptrfiletable->readoffset = 0;
    UFDTArr[fd].ptrfiletable->writeoffset = 0;
    UFDTArr[fd].ptrfiletable->FileActualSize = 0;
}
    ...
    while(1)
    {
        ...
        else if(count == 4)
        {
            if(strcmp(command[0],"Iseek") == 0)
            {
                ...
                if(ret == -1)
                {
                    printf("ERROR : Unable to oerform lseek\n");
                }
            }
            else
            {
